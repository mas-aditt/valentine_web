...
function Question() {
  ...
  const trends = ['Kenapa pacarku sangat lucu?', 'Mengapa Aditya sangat mencintai Dinda?', `Kenapa Dinda sangat cantik dan gemass?`,]

  const handleSearch = (e) => {
    e.preventDefault()
    if (searchQuery.toLowerCase() === 'berapa lama kita sudah bersama?' || searchQuery.toLowerCase() === 'berapa lama kita sudah bersama') {
          navigate("/timer");
       ...
       <div
        ...
        <div className="flex flex-col min-h-screen w-full max-w-[480px] mx-auto ">
          ...
          <div className='mt-10  px-5 text-white/50'>
            <p>Tips : coba searching "berapa lama kita sudah bersama?" : ) </p>
          </div>
        </div>
      )}
    </div>
