      ...
      if (passcode.length < 6) {
        ...
        if (newPasscode.length === 4) {
          ...
          if (enteredPasscode === CORRECT_PASSCODE) {
            ...
            setMessage('Passcode Salah , clue: our anniversary date!')
            setTimeout(() => {
              setPasscode([])
              setMessage('')
            }, 1800)
  ...
  return (
    <div className="min-h-screen w-full bg-black/60 backdrop-blur-sm text-white flex items-center justify-center overflow-hidden">
        <div className="flex flex-col items-center max-w-full">
            {/* Title */}
            <h1 className="text-2xl font-light mb-8">Masukkan Passcode</h1>
            ...
            {/* Message */}
            {message && (
            <div className={`mb-4 -mt-9 text-sm font-bold ${message === 'Yeayy Benarr !! :)' ? 'text-green-500' : 'text-red-500'}`}>
                {message}
            </div>
            )}
